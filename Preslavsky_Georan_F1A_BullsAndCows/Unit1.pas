...
//Fonction pour vérifier si le caractère est une lettre de l'alphabet
...
// Fonction pour vérifier tout le mot , qui va vrifier chaque caractère , via la fonction EstLettre
...
//procedure du boutton Go quand on joue
    ...
    begin
      ...
      if (StrtoInt(EBulls.Text)= cpt1) then
        begin
          ...
          // si on veut une fenetre pour dire si on a gagné ou perdu , avec un boutton recommencer
          {
...
procedure TForm1.BQuitClick(Sender: TObject);  //Boutton quitter
begin
...
procedure TForm1.BRazClick(Sender: TObject); //Boutton Recommencer (Restar)
begin
  BGo.Visible:=true;
  RandMot(Sender);
...
procedure TForm1.FormCreate(Sender: TObject); // quand le jeu est créé , il y aura cela de base
begin
